...
- 在 iCloud、OneDrive 等 Finder Sync 菜单不可用的位置，可配置修饰键右键、鼠标中键或触控板三指作为备用入口
...
`make install` 会安装到 `/Applications/灵犀右键.app`（Finder Sync 扩展需要标准应用目录才能被系统稳定发现）。安装后在“系统设置 → 通用 → 登录项与扩展 → 文件提供程序/访达扩展”中启用“灵犀右键 Finder 扩展”。也可以在应用的“通用设置”页打开系统扩展管理界面。备用入口按需启用：修饰键右键和中键监听需要“辅助功能”权限，首次读取 Finder 当前选择时还会请求“自动化 → Finder”权限；三个入口默认均关闭。
...
Finder Sync、Core Graphics 事件监听和 AppKit 菜单均使用 Apple 公共 API，但系统要求用户显式启用扩展。云盘备用入口只在用户主动开启后监听，并只在 Finder 位于 iCloud、OneDrive 或 `~/Library/CloudStorage` 文件提供程序目录时接管；其他位置会转发原始鼠标操作。彻底删除默认要求二次确认，写权限操作只补充所有者写入位，解散目录包含名称冲突处理与失败回滚；没有复刻原应用的付费、推广和第三方软件入口。翻译动作会把用户选中的文本作为查询参数发送到所选翻译网站，二维码生成完全离线。
